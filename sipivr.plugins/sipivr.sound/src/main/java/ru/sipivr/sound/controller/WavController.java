...
public class WavController extends BaseController {
    ...
    public void uploadFiles(@RequestParam("path") String path, @RequestParam("files") List<MultipartFile> files) throws Exception {
        for (MultipartFile file : files) {
            ...
            String target = appConfig.getSoundPath(path, file.getOriginalFilename());
            File targetFile = new File(target);
            file.transferTo(targetFile);
    ...
    public void rename(@RequestParam("path") String path, @RequestParam("newName") String newName) throws Exception {
        Path source = Paths.get(appConfig.getSoundPath(path));
        Path target = Files.move(source, source.resolveSibling(newName), StandardCopyOption.REPLACE_EXISTING);

        if (!target.toFile().isDirectory()) {
            File mp3File = new File(target.toString().replace(".wav", ".mp3"));
            if (mp3File.exists()) {
                mp3File.delete();
            }
        }
        if (!source.toFile().isDirectory()) {
            File mp3File = new File(source.toString().replace(".wav", ".mp3"));
            if (mp3File.exists()) {
                mp3File.delete();
            }
        }
    ...
    public void delete(@RequestParam("path") String path) throws Exception {
        String targetPath = appConfig.getSoundPath(path);
        File target = new File(targetPath);

        target.delete();

        if (!target.isDirectory()) {
            File mp3File = new File(targetPath.replace(".wav", ".mp3"));
            if (mp3File.exists()) {
                mp3File.delete();
            }
        }
